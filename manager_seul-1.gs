...
function redirect(url) {
  var html = "<!DOCTYPE html><html><head><meta charset='UTF-8'><meta name='viewport' content='width=device-width,initial-scale=1'>";
  html += "<style>body{background:#0a0a0a;display:flex;align-items:center;justify-content:center;min-height:100vh;font-family:sans-serif}</style></head>";
  html += "<body><script>window.location.replace('" + url + "');<\/script>";
  html += "<a href='" + url + "' style='color:#c9a84c;font-size:16px;font-weight:700'>Cliquez ici si la page ne charge pas</a>";
  html += "</body></html>";
  return HtmlService.createHtmlOutput(html).setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL);
}
