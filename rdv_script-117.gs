...
function doGet(e) {
  ...
  if (p==="theme") return pageTheme(e);
  if (p==="savetheme") return pageSaveTheme(e);
  if (p==="annuler_client") return pageAnnulerClient(e);
  if (p==="confirmer_annulation") return pageConfirmerAnnulation(e);
...
// ═══ THEME ═══
function pageTheme(e) {
  var url = ScriptApp.getService().getUrl();
  if (e.parameter.pwd !== CONFIG.MOT_DE_PASSE) return err("Erreur", url, e.parameter.pwd);
  var couleur = getConfig("theme_couleur") || "#6366F1";
  var themes = [
    {nom:"Violet", val:"#6366F1"},
    {nom:"Bleu", val:"#3B82F6"},
    {nom:"Rose", val:"#EC4899"},
    {nom:"Vert", val:"#10B981"},
    {nom:"Orange", val:"#F59E0B"},
  ...
  var themesHTML = themes.map(function(t) {
    var sel = t.val === couleur ? "border:3px solid #fff;" : "";
    return '<label style="display:flex;flex-direction:column;align-items:center;gap:5px;cursor:pointer">'
      + '<input type="radio" name="couleur" value="' + t.val + '"' + (t.val===couleur?" checked":"") + ' style="display:none">'
      + '<span style="width:48px;height:48px;background:' + t.val + ';border-radius:12px;display:block;' + sel + '"></span>'
      + '<span style="font-size:11px;color:#888">' + t.nom + '</span>'
      + '</label>';
  }).join("");
  ...
  var h = '<!DOCTYPE html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><meta charset="UTF-8">'
    + '<base target="_top">'
    + '<style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:sans-serif;background:#0A0A0F;color:#fff}'
    + '.hdr{background:linear-gradient(135deg,' + couleur + ',' + couleur + 'cc);padding:16px;display:flex;justify-content:space-between;align-items:center}'
    + '.hdr h1{font-size:17px;font-weight:700}.hdr a{color:rgba(255,255,255,.85);font-size:12px;text-decoration:none;background:rgba(255,255,255,.2);padding:7px 14px;border-radius:20px}'
    + '.ct{padding:16px}.sec{background:#1a1a2e;border-radius:14px;padding:16px;margin-bottom:12px;border:1px solid #21213A}'
    + '.st{font-size:11px;font-weight:700;color:' + couleur + ';text-transform:uppercase;letter-spacing:1px;margin-bottom:14px}'
    + '.cgrid{display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin-bottom:4px}'
    + '.sbtn{width:100%;padding:15px;background:' + couleur + ';color:#fff;border:none;border-radius:10px;font-size:16px;font-weight:700;cursor:pointer;margin-top:6px;font-family:sans-serif}'
    + '.rbtn{display:block;text-align:center;padding:12px;background:#1a1a2e;color:#888;border-radius:10px;font-size:14px;text-decoration:none;margin-top:10px;border:1px solid #21213A}'
    + '.safe{height:30px}'
    + '</style></head><body>'
    + '<div class="hdr"><h1>🎨 Theme</h1><a href="' + url + '?page=admin&pwd=' + e.parameter.pwd + '">Retour</a></div>'
    + '<div class="ct">'
    + '<div class="sec"><div class="st">Couleur principale</div>'
    + '<form id="tf" action="' + url + '" method="get">'
    + '<input type="hidden" name="page" value="savetheme">'
    + '<input type="hidden" name="pwd" value="' + e.parameter.pwd + '">'
    + '<div class="cgrid">' + themesHTML + '</div>'
    + '<input type="submit" value="Appliquer" style="width:100%;padding:15px;background:#6366F1;color:#fff;border:none;border-radius:10px;font-size:16px;font-weight:700;cursor:pointer;margin-top:14px;font-family:sans-serif">'
    + '</form></div>'
    + '<div class="sec"><div class="st">Photo du salon</div>'
    + '<input type="text" id="photoUrl" placeholder="https://i.imgur.com/..." value="' + (getConfig("photo_url")||"https://i.imgur.com/s2frgkQ.jpg") + '" style="width:100%;padding:13px;background:#111;border:1.5px solid #21213A;border-radius:10px;color:#fff;font-size:13px;font-family:sans-serif;margin-bottom:10px">'
    + '<a href="' + url + '?page=admin&pwd=' + e.parameter.pwd + '" id="savePhotoBtn" style="display:block;text-align:center;padding:12px;background:#6366F1;color:#fff;border-radius:10px;font-size:14px;font-weight:700;text-decoration:none">Sauvegarder la photo</a>'
    + '</div>'
    + '<a href="' + url + '?page=admin&pwd=' + e.parameter.pwd + '" class="rbtn">Retour sans sauvegarder</a>'
    + '<div class="safe"></div>'
    + '</div></body></html>';
...
function pageSaveTheme(e) {
  var url = ScriptApp.getService().getUrl();
  if (e.parameter.pwd !== CONFIG.MOT_DE_PASSE) return err("Erreur", url, e.parameter.pwd);
  setConfig("theme_couleur", e.parameter.couleur || "#6366F1");
  return HtmlService.createHtmlOutput(
    '<html><head><base target="_top"></head>'
    + '<body style="font-family:sans-serif;background:#0A0A0F;color:#fff;min-height:100vh;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:24px;text-align:center">'
    + '<div style="font-size:64px;margin-bottom:16px">🎨</div>'
    + '<div style="font-size:20px;font-weight:800;color:#10B981;margin-bottom:16px">Theme applique !</div>'
    + '<a href="' + url + '?page=admin&pwd=' + e.parameter.pwd + '" style="display:block;padding:14px 28px;background:#6366F1;color:#fff;border-radius:10px;text-decoration:none;font-weight:700;font-size:16px">Retour admin</a>'
    + '</body></html>'
  ).setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL);
}
...
// ═══ ANNULATION CLIENT ═══
function pageAnnulerClient(e) {
  var url = ScriptApp.getService().getUrl();
  var h = '<!DOCTYPE html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><meta charset="UTF-8">'
    + '<base target="_top">'
    + '<style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:sans-serif;background:#0F0F1A;color:#fff;min-height:100vh}'
    + '.hdr{background:linear-gradient(135deg,#EF4444,#DC2626);padding:20px;text-align:center}'
    + '.hdr h1{font-size:18px;font-weight:800}.hdr p{font-size:12px;color:rgba(255,255,255,.7);margin-top:4px}'
    + '.ct{padding:20px}'
    + '.sec{background:#161622;border-radius:14px;padding:16px;margin-bottom:14px;border:1px solid #21213A}'
    + '.st{font-size:11px;font-weight:700;color:#EF4444;text-transform:uppercase;letter-spacing:1px;margin-bottom:12px}'
    + 'label{display:block;font-size:11px;color:#888;text-transform:uppercase;margin-bottom:6px}'
    ...
    + '.sbtn{width:100%;padding:15px;background:#EF4444;color:#fff;border:none;border-radius:10px;font-size:16px;font-weight:700;cursor:pointer;margin-top:14px;font-family:sans-serif}'
    + '.rbtn{display:block;text-align:center;padding:12px;background:#161622;color:#888;border-radius:10px;font-size:14px;text-decoration:none;margin-top:10px;border:1px solid #21213A}'
    + '.safe{height:30px}'
    + '</style></head><body>'
    + '<div class="hdr"><h1>❌ Annuler mon RDV</h1><p>Entrez votre reference de reservation</p></div>'
    + '<div class="ct">'
    + '<div class="sec"><div class="st">Votre reference</div>'
    + '<form id="caf" action="' + url + '" method="get">'
    + '<input type="hidden" name="page" value="confirmer_annulation">'
    + '<label>Reference (ex: RDV-0001)</label>'
    + '<input type="text" name="ref" placeholder="RDV-0001" required>'
    + '<label style="margin-top:12px">Votre telephone</label>'
    + '<input type="tel" name="tel" placeholder="06 00 00 00 00" required>'
    + '<input type="submit" value="Annuler mon RDV" style="width:100%;padding:15px;background:#EF4444;color:#fff;border:none;border-radius:10px;font-size:16px;font-weight:700;cursor:pointer;margin-top:14px;font-family:sans-serif">'
    + '</form></div>'
    + '<a href="' + url + '" class="rbtn">Retour accueil</a>'
    + '<div class="safe"></div>'
    + '</div></body></html>';
  return HtmlService.createHtmlOutput(h).setTitle("Annuler - " + CONFIG.NOM).setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL);
}

function pageConfirmerAnnulation(e) {
  var url = ScriptApp.getService().getUrl();
  var ref = (e.parameter.ref || "").toUpperCase().trim();
  var tel = (e.parameter.tel || "").replace(/ /g, "");
  var ws = getSheetRDV();
  if (ws.getLastRow() < 2) {
    return HtmlService.createHtmlOutput('<html><head><base target="_top"></head><body style="font-family:sans-serif;background:#0F0F1A;color:#fff;padding:24px;text-align:center"><p>RDV introuvable</p><a href="' + url + '" style="color:#6366F1">Retour</a></body></html>').setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL);
  }
  var vals = ws.getDataRange().getValues();
  for (var i = 1; i < vals.length; i++) {
    var id = String(vals[i][0]).toUpperCase().trim();
    var telRDV = String(vals[i][4]).replace(/ /g, "");
    if (id === ref && telRDV === tel && vals[i][9] === "Confirme") {
      ws.getRange(i+1, 10).setValue("Annule");
      ws.getRange(i+1, 10).setBackground("#f8d7da").setFontColor("#c0392b");
      return HtmlService.createHtmlOutput(
        '<html><head><base target="_top"><meta name="viewport" content="width=device-width,initial-scale=1"></head>'
        + '<body style="font-family:sans-serif;background:#0F0F1A;color:#fff;min-height:100vh;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:24px;text-align:center">'
        + '<div style="font-size:64px;margin-bottom:16px">✅</div>'
        + '<div style="font-size:20px;font-weight:800;color:#10B981;margin-bottom:8px">RDV annule !</div>'
        + '<div style="font-size:13px;color:#888;margin-bottom:24px">Votre RDV ' + ref + ' a bien ete annule</div>'
        + '<a href="' + url + '" style="display:block;padding:14px 28px;background:#6366F1;color:#fff;border-radius:10px;text-decoration:none;font-weight:700;font-size:16px">Retour accueil</a>'
        + '</body></html>'
      ).setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL);
    }
  }
  return HtmlService.createHtmlOutput(
    ...
    + '<body style="font-family:sans-serif;background:#0F0F1A;color:#fff;min-height:100vh;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:24px;text-align:center">'
    + '<div style="font-size:64px;margin-bottom:16px">❌</div>'
    + '<div style="font-size:20px;font-weight:800;color:#EF4444;margin-bottom:8px">RDV introuvable</div>'
    + '<div style="font-size:13px;color:#888;margin-bottom:24px">Reference ou telephone incorrect</div>'
    + '<a href="' + url + '?page=annuler_client" style="display:block;padding:14px 28px;background:#EF4444;color:#fff;border-radius:10px;text-decoration:none;font-weight:700;font-size:16px;margin-bottom:10px">Reessayer</a>'
    + '<a href="' + url + '" style="display:block;padding:14px 28px;background:#1a1a2e;color:#888;border-radius:10px;text-decoration:none;font-weight:700;font-size:16px;border:1px solid #21213A">Retour accueil</a>'
    + '</body></html>'
  ).setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL);
}
...
function pageAdmin(e) {
  ...
  var html='<!DOCTYPE html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><meta charset="UTF-8"><title>Admin</title>'
    ...
    +'<a href="'+url+'?page=theme&pwd='+pwd+'" style="display:block;padding:16px;background:#1a1a2e;border-radius:12px;margin-bottom:10px;text-decoration:none;color:#fff;border:1.5px solid #6366F1;font-size:15px;font-weight:600" target="_top">🎨 Changer le theme</a>'
    +'<a href="'+url+'?page=annuler_client" style="display:flex;align-items:center;gap:10px;background:#1a1a2e;border-radius:12px;padding:14px;text-decoration:none;color:#fff;border:1px solid #EF4444;margin-bottom:10px"><span style="font-size:24px">❌</span><div><div style="font-size:14px;font-weight:700">Annuler mon RDV</div><div style="font-size:12px;color:#888">Annulez avec votre reference</div></div><span style="margin-left:auto;color:#EF4444">›</span></a>'
...
function pageHome() {
  ...
  var html='<!DOCTYPE html><html><head>'
    ...
    +'<meta name="apple-mobile-web-app-title" content="'+CONFIG.NOM+'">'
    +'<link rel="apple-touch-icon" href="https://i.imgur.com/s2frgkQ.jpg">'
    +'<style>'
    ...
    +'<div style="border-radius:14px;overflow:hidden;margin-bottom:12px;max-height:200px">'
    +'<img src="https://i.imgur.com/s2frgkQ.jpg" style="width:100%;object-fit:cover;max-height:200px" alt="' + CONFIG.NOM + '">'
    +'</div>'
    ...
    +'<div id="installDiv" style="display:none;padding:10px 14px">'
    +'<button id="installBtn" style="width:100%;padding:12px;background:#1a1a2e;color:#6366F1;border:1.5px solid #6366F1;border-radius:10px;font-size:14px;font-weight:700;cursor:pointer;font-family:sans-serif">📱 Installer l app</button>'
    +'</div>'
    +'<script>'
    +'var deferredPrompt;'
    ...
    +'    if(deferredPrompt){deferredPrompt.prompt();deferredPrompt.userChoice.then(function(){deferredPrompt=null;document.getElementById("installDiv").style.display="none";});}'
    +'  });}'
    +'});'
    +'</script>'
    +'<script>'
