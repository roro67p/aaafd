...
function pageAjoutCoiff(e) {
  ...
  var h='<!DOCTYPE html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><meta charset="UTF-8">'
    ...
    + '<button type="button" class="sbtn" onclick="document.getElementById(\'sf\').submit()">Enregistrer</button>'
    +'</form>'
    +'<a href="'+url+'?page=admin&pwd='+e.parameter.pwd+'" class="rbtn">Retour</a>'
    +'</div></body></html>';
  return HtmlService.createHtmlOutput(h).setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL);
...
function pageAvis(e) {
  ...
  var h = '<!DOCTYPE html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><meta charset="UTF-8">'
    ...
    + '<input type="submit" value="Envoyer mon avis" class="sbtn" onclick="this.disabled=true">'
    + '</form>'
...
function pageAdmin(e) {
  ...
  var html='<!DOCTYPE html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><meta charset="UTF-8"><title>Admin</title>'
    ...
    +'</div></div>'
    +'<div class="safe"></div>'
    +'<script>function sw(n,el){for(var i=0;i<7;i++){var p=document.getElementById("p"+i);if(p)p.className="pane"+(i===n?" show":"");}document.querySelectorAll(".tab").forEach(function(t,i){t.className="tab "+(i===n?"on":"off");});}</script>'
    +'</body></html>';
...
function pageHome() {
  ...
  var html='<!DOCTYPE html><html><head>'
    ...
    +'<a href="'+url+'?page=avis" style="display:flex;align-items:center;gap:10px;background:#1a1a2e;border-radius:12px;padding:14px;text-decoration:none;color:#fff;border:1px solid #21213A;margin-bottom:12px"><span style="font-size:24px">⭐</span><div><div style="font-size:14px;font-weight:700">Avis clients</div><div style="font-size:12px;color:#888">Voir et laisser un avis</div></div><span style="margin-left:auto;color:#6366F1">›</span></a>'
