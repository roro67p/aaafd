...
function pageConfirmer(e) {
  ...
  // Couleur verte pour RDV confirme
  var lastRow = ws.getLastRow();
  ws.getRange(lastRow, 1, 1, 11).setBackground("#d4edda");
...
function pageColorier(e) {
  ...
  for (var i = 0; i < vals.length; i++) {
    ...
    if (statut === "Annule") {
      ws.getRange(row, 1, 1, 11).setBackground("#f8d7da");
    } else if (statut === "Confirme") {
      var col = coiffColors[coiff] || "#d4edda";
      ws.getRange(row, 1, 1, 11).setBackground(col);
    }
  }
